...
export class GridLayoutService {
    ...
    emitLayoutElement(id:number, w:number, h:number, x:number, y:number,
                      minH:number = 2, minW:number = 4, maxH:number = 6, maxW:number = 12,
                      moved:boolean = false) {
        /* set new element or update, if element exist */
        this._dataStore.gridState.layout[id] = {w, h, x, y, id, minH, minW, maxH, maxW, moved};
    ...
    createStyle(pos:IPosition) {
        return this._utiles.setTransform(pos);
    }
